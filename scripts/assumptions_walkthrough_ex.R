...
data(economics)       # bad example (all are above the blue lines)
...
read_csv(file = "./data/untidy/mtcars_raw.csv") #para leer data que ya tenemos
...
cars_temp %>% 
  ...
  arrange(sd) #cambia el orden - lo pone de menor a mayor según la SD

  cars_temp %>% 
    group_by(cyl) %>%
    summarize(avg = mean(mpg), sd = sd(mpg)) %>%
    arrange(sd) %>%
    write_csv(file = "./data/tidy/mtcars_tidy.csv")
...
# lo de acontinuación es igual que lo de antes, es otro modo de escribirlo
cars_temp %>% 
  group_by(cyl) %>%
  summarize(avg = mean(mpg), sd = sd(mpg)) %>%
  arrange(sd) %>%
...
read.csv = here("data", "tidy", "mtcars_tidy.csv")
